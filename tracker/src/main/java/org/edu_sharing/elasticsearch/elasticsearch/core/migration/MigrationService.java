...
public class MigrationService {
    ...
    public void runMigration() throws IOException, InterruptedException {
        ...
        Set<Class<? extends TrackerConfig<?, ? extends CommitTimeStatus>>> migrationTracker = IntStream.range(startIndex, migrationInfos.size())
    ...
    class MigrationJobRunner {
        ...
        MigrationJobRunner(
            ...
            jobs = Stream.of( // Jobs needs to be ordered by MigrationStep (see requires migration)
                            ...
                            // no shouldMigrateAuthorities guard needed: ReindexMigrationJob.onEnterState already
                            // skips itself when the source index doesn't exist (e.g. migrating from 9.0/9.1)
                            new ReindexMigrationJob(MigrationStep.REINDEX_AUTHORITIES_INDEX_PROGRESS_STEP, client, context.getSourceAuthoritiesIndex(), context.getTargetAuthoritiesIndex(),authorityMigrationScript,reindexBatchSize,requestsPerSecond, tickService),
