...
public class DocumentsMigrationJob implements MigrationJob {
    ...
    private final Collection<Class<? extends TrackerConfig<?, ? extends CommiteTimeStatus>>> migrationTrackerConfigTypes;
    ...
    private <STATE> Long getIndexStateCommitTime(MigrationContext context, TrackerConfig<?, STATE> trackerConfig) {
        if (!CommiteTimeStatus.class.isAssignableFrom(trackerConfig.getStatusClass())) {
            return null;
        }
        ...
        try {
            return ((CommiteTimeStatus) stateService.getState()).getCommitTime();
        } catch (IOException e) {
